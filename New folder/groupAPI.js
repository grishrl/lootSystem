            ...
            customMiddleWare.checkAdmin(id,results.admins,results.creator).then(
              function(userLevel){
                //    console.log(results);
                    ...
                    res.status(400).send({message:'Creator user cannot be deleted.',code:00});
    ...
    for(var i=0;i<res.length;i++){
      ...
      list.save().then(
        ...
        insertHistory(myobj).then(function(resp){
          //continue operation - operation pases back to the remove user URI
        }
        ,function(resp){
          console.log(resp);
        })
